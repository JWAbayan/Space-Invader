...
from Entity import Player, Bullet, Enemy, States, Actions
...
class SpaceAI:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RIGHT:
                    self.player.direction = Actions.MOVE_RIGHT.value

                # LEFT MOVEMENT
                if event.key == pygame.K_LEFT:
                    self.player.direction = Actions.MOVE_LEFT.value
            ...
            if event.type == pygame.KEYUP:
                self.player.state = States.READY
                if event.key == pygame.K_LEFT or event.key == pygame.K_RIGHT:
                    self.player.direction = Actions.STOP.value
    ...
    def on_render(self):
        ...
        if self.player.state == States.MOVING:
            self.player.move_hori(
                self.screenWidth, self.game_clock.get_time())
    ...
    def spawn_enemy(self):
        spawn_timer = (pygame.time.get_ticks() - self.game_start_ticks)/5000
        increase_level_timer = (pygame.time.get_ticks() - self.game_start_ticks) / 1000

        if(increase_level_timer % 5 == 0):
            self.spawn_enemy_total += 5
